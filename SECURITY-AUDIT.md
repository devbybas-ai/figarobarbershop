...
| JS bundle analysis: no secrets in client bundle           | Critical | Pass -- server env vars not prefixed with NEXT*PUBLIC* (except APP_URL) |
...
| Environment variables: NEXT*PUBLIC* only for public values | Critical | Pass -- only APP_URL and Stripe publishable key        |
